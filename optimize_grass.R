...
run_rpops <- function(config, treatment_raster = NULL) {
...
grass_lazy_init <- function(infected_file,
                            host_file,
                            total_file,
                            weather_file,
                            quarantine_file = "") {
  ...
  if (!file.exists(weather_list_file)) {
    ...
    if (quarantine_areas_file != "") {
      execGRASS("r.in.gdal",
        input = quarantine_areas_file,
        output = quarantine,
        flags = c("quiet", "o")
      )
    }
...
run_r.pops.spread <- function(parameters, treatment_raster = NULL) {
  grassparams <- grass_lazy_init(
    ...
    parameters$quarantine_areas_file
  )
  if (!is.null(treatment_raster)) {
  ...
  quarantine_output <- file.path(tempdir(), "quarantine_output.txt")
  ...
  if (parameters$quarantine_areas_file != "") {
    params$quarantine <- grassparams$quarantine
    params$quarantine_output <- quarantine_output
    params$quarantine_directions <- parameters$quarantine_directions
  }
  do.call(execGRASS, params)

  results <- list()
  if (parameters$quarantine_areas_file != "") {
    df <- read.csv(params$quarantine_output)
    df <- df[nrow(df), grepl("dist", names(df))]
    # quarantine escape will be distance 0
    df[is.na(df)] <- 0
    results$quarantine_distance <- (mean(as.matrix(df), na.rm = TRUE))
  }

  infected_area <- execGRASS("r.info",
    map = grassparams$average, flags = c("g", "e"),
    intern = TRUE
  )
  infected_area <- tail(strsplit(infected_area[[26]], " ")[[1]], 1)
  results$infected_area <- as.numeric(gsub("\"", "", infected_area))
