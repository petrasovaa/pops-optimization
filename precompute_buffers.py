...
def run_buffer(params):
    x, y, column, buffer, region_coef = params
...
if __name__ == "__main__":
    ...
    for line in data:
        x, y, cat = line.split(",")
        params.append((float(x), float(y), cost, buffer, region_coef))
    with multiprocessing.Pool(processes=nproc) as pool:
        ...
        with open("buffers.csv", "w") as f:
            for each in results:
                ...
                f.write("\n")
        gs.run_command(
            "r.in.xyz", input="buffers.csv", output="buffer_cost", separator="comma"
        )
